...
class MyComponent extends React.Component{
  ...
  ///// CLEAR EVERYTHING
  clearHandler() {this.setState(defaultState)};
  ...
  inputOperationHandler(op){
    if(!this.state.currValue && op === "-") return this.setState({...this.state, currValue: "-"})
    ...
    if(!this.state.operator && !this.state.prevValue) 
      return this.setState({...this.state, operator: op, prevValue: this.state.currValue, currValue: null});

    if(this.state.operator){
      if(this.state.prevValue){
        let updatedResult = solve(+this.state.prevValue, +this.state.currValue, this.state.operator)
        return this.setState({...this.state, operator: op, prevValue: updatedResult, currValue: null});
      }
      ...
      <div id="calculator">
        ...
        <div id="display">
          ...
          <div id="display_input">
            ...
            {!this.state.currValue && !this.state.prevValue ? "0" : `= ${this.state.currValue}`}
          </div>
        </div>
        ...
        <div id="calculator__btns">
          ...
          <button id="divide"   onClick={() => this.inputOperationHandler("/")}>÷</button>
          <button id="one"      onClick={() => this.inputNumberHandler("1")}>1</button>
          <button id="two"      onClick={() => this.inputNumberHandler("2")}>2</button>
          <button id="three"    onClick={() => this.inputNumberHandler("3")}>3</button>
          <button id="multiply" onClick={() => this.inputOperationHandler("x")}>×</button>
          <button id="four"     onClick={() => this.inputNumberHandler("4")}>4</button>
          <button id="five"     onClick={() => this.inputNumberHandler("5")}>5</button>
          <button id="six"      onClick={() => this.inputNumberHandler("6")}>6</button>
          <button id="subtract" onClick={() => this.inputOperationHandler("-")}>-</button>
          <button id="seven"    onClick={() => this.inputNumberHandler("7")}>7</button>
          <button id="eight"    onClick={() => this.inputNumberHandler("8")}>8</button>
          <button id="nine"     onClick={() => this.inputNumberHandler("9")}>9</button>
          <button id="add"      onClick={() => this.inputOperationHandler("+")}>+</button>
          <button id="decimal"  onClick={this.inputDecimalHandler}>.</button>
          <button id="zero"     onClick={() => this.inputNumberHandler("0")}>0</button>
